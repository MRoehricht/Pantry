...
using Pantry.Module.Good.Metrics;
...
public static class GoodsEndpoint
{
    public static RouteGroupBuilder MapGoodsEndpoint(this RouteGroupBuilder group)
    {
        ...
        group.MapGet("/", GetGoods).WithName("GetGoods").Produces<IEnumerable<Shared.Models.GoodModels.Good>>(StatusCodes.Status200OK).WithTags("Goods").WithOpenApi();
        group.MapGet("/{id}", GetGood).WithName("GetGoodById").Produces<Shared.Models.GoodModels.Good>(StatusCodes.Status200OK).Produces(StatusCodes.Status404NotFound).WithTags("Goods").WithOpenApi();
        group.MapPost("/", CreateGood).WithName("CreateGood").Produces<Shared.Models.GoodModels.Good>(StatusCodes.Status201Created).WithTags("Goods").WithOpenApi();
