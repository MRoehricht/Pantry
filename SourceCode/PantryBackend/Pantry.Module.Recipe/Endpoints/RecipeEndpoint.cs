...
public static class RecipeEndpoint
{
    ...
    {
        group.MapGet("/overview/{id}", GetRecipeOverviewById).WithName("GetRecipeOverviewById")
            .Produces<IList<RecipeOverview>>().Produces(StatusCodes.Status404NotFound).WithOpenApi();
        group.MapGet("/overview/", GetRecipeOverview).WithName("GetRecipeOverview").Produces<IList<RecipeOverview>>()
            .WithOpenApi();
        group.MapGet("/", GetRecipes).WithName("GetRecipes").Produces<IList<Shared.Models.RecipeModels.Recipe>>()
            .WithOpenApi();
        group.MapGet("/{id}", GetRecipe).WithName("GetRecipeById").Produces<Shared.Models.RecipeModels.Recipe>()
            .Produces(StatusCodes.Status404NotFound).WithOpenApi();
        group.MapPost("/", CreateRecipe).WithName("CreateRecipe")
            .Produces<Shared.Models.RecipeModels.Recipe>(StatusCodes.Status201Created).WithOpenApi();
        group.MapPut("/", UpdateRecipe).WithName("UpdateRecipe").Produces(StatusCodes.Status204NoContent)
            .Produces(StatusCodes.Status404NotFound).WithOpenApi();
        group.MapDelete("/{id}", DeleteRecipe).WithName("DeleteRecipe").Produces(StatusCodes.Status204NoContent)
            .Produces(StatusCodes.Status404NotFound).WithOpenApi();

        return group;
    }

    private static async Task<IResult> UpdateRecipe(IHeaderEMailService eMailService, RecipeContext context,
    ...
    private static async Task<IResult> DeleteRecipe(IPublishEndpoint publishEndpoint, IHeaderEMailService eMailService,
        RecipeContext context, Guid id)
    {
        var eMail = eMailService.GetHeaderEMail();
        if (string.IsNullOrEmpty(eMail)) return Results.Unauthorized();
    ...
    private static async Task<IResult> CreateRecipe(IHeaderEMailService eMailService, RecipeContext context,
    ...
    private static async Task<IResult> GetRecipeOverviewById(IHeaderEMailService eMailService, RecipeContext context,
        Guid id)
    {
        var eMail = eMailService.GetHeaderEMail();
        if (string.IsNullOrEmpty(eMail)) return Results.Unauthorized();
